...
export function BJGame({G,setG,history,addHistory,user}){
  ...
  async function deal(){
    if(submittingR.current || bj.active) return;
    ...
    try{
      ...
      if(useServer){
        let data,error;
        try{
          ({ data, error } = await supabase.rpc('bj_deal', { p_user_id: user.id, p_bet: bet }));
        }catch(e){
          console.error('[bj_deal] exceção:',e);
          setMsg("⚠️ Erro de conexão ao distribuir. Tente novamente.");setMT("loss");
          return;
        }
        if(error || !data || !data[0]){console.error('[bj_deal]',error);setMsg(error?.message?.includes('em andamento')?"⚠️ Já tem uma rodada em andamento.":"⚠️ Erro ao distribuir. Tente novamente.");setMT("loss");return;}
        const r=data[0];
  ...
  async function hit(){
    ...
    if(useServer){
      let data,error;
      try{
        ({ data, error } = await supabase.rpc('bj_hit', { p_user_id: user.id, p_round_id: bj.roundId }));
      }catch(e){
        console.error('[bj_hit] exceção:',e);
        setMsg("⚠️ Erro de conexão ao pedir carta. Tente novamente.");setMT("loss");
        return;
      }
  ...
  async function dbl(){
    ...
    if(useServer){
      let data,error;
      try{
        ({ data, error } = await supabase.rpc('bj_double', { p_user_id: user.id, p_round_id: bj.roundId }));
      }catch(e){
        console.error('[bj_double] exceção:',e);
        setMsg("⚠️ Erro de conexão ao dobrar. Tente novamente.");setMT("loss");
        return;
      }
  ...
  async function stand(){
    ...
    if(useServer){
      ...
      let data,error;
      try{
        ({ data, error } = await supabase.rpc('bj_stand', { p_user_id: user.id, p_round_id: bj.roundId }));
      }catch(e){
        console.error('[bj_stand] exceção:',e);
        setMsg("⚠️ Erro de conexão ao parar. Tente novamente.");setMT("loss");
        return;
      }
