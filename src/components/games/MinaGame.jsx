...
export function MinaGame({G,setG,history,addHistory,user,demoMode}){
  ...
  async function start(){
    ...
    if(useServer){
      ...
      if(error || !data){console.error('[mina_start]',error);if(error?.message?.includes("em andamento")){setStuck(true);setMsg("⚠️ Rodada anterior travada detectada.");}else{setMsg("⚠️ Erro ao iniciar. Tente novamente.");}setMT("loss");return;}
  ...
  async function tap(i){
    ...
    if(useServer){
      const { data, error } = await supabase.rpc('mina_tap', { p_user_id: user.id, p_round_id: mn.roundId, p_cell: i });
  ...
  async function saque(){
    ...
    const useServer = hasSupabase && user && !demoMode;
    if(useServer){
      const { data, error } = await supabase.rpc('mina_cashout', { p_user_id: user.id, p_round_id: mn.roundId });
  ...
  async function unstick(){
    if(!user) return;
    setStuck(false);
    const { error } = await supabase.rpc('abandon_round', { p_user_id: user.id, p_game: 'mina' });
