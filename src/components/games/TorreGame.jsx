...
export function TorreGame({G,setG,history,addHistory,user}){
  ...
  async function begin(){
    if(submittingR.current || tw.active) return;
    ...
    try{
      ...
      if(useServer){
        ...
        if(error || !data){console.error('[torre_start]',error);setMsg(error?.message?.includes('em andamento')?"⚠️ Já tem uma rodada em andamento.":"⚠️ Erro ao iniciar. Tente novamente.");setMT("loss");return;}
  ...
  async function step(f,c){
    ...
    if(useServer){
      let data,error;
      try{
        ({ data, error } = await supabase.rpc('torre_step', { p_user_id: user.id, p_round_id: tw.roundId, p_cell: c }));
      }catch(e){
        console.error('[torre_step] exceção:',e);
  ...
  async function saque(){
    ...
    if(useServer){
      let data,error;
      try{
        ({ data, error } = await supabase.rpc('torre_cashout', { p_user_id: user.id, p_round_id: tw.roundId }));
      }catch(e){
        console.error('[torre_cashout] exceção:',e);
