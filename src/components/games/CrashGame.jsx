...
//  Correção:
//  - P(crash >= x) = 0.93/x  (house edge 7%, RTP ~93%)
//  - Capped em 50× máximo
...
export function CrashGame({G,setG,history,addHistory,user,demoMode}){
  ...
  async function cashOut(){
    ...
    if(useServer){
      ...
      if(error || !data || !data[0]){console.error('[crash_cashout]',error);setMsg("⚠️ Erro ao sacar. Tente novamente.");setMT("loss");return;}
      const r=data[0];
      setCr(p=>({...p,betIn:false,running:false}));
      if(r.won){
        sCrashStop();sCash();
