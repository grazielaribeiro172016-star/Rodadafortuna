...
export function useGameSync(user) {
  ...
  // syncProfile foi removida: fazia update direto na tabela profiles do
  // client, sem passar por RPC — era a brecha que permitia qualquer usuário
  // logado alterar seu próprio saldo/streak/stats via console do navegador.
  // Nenhum jogo dependia dela (todos já passam syncOpts pra syncRound, que
  // usa a RPC segura update_balance_safe). Ver supabase_lock_profile_writes.sql
  // pra saber a trava correspondente aplicada no banco.
  ...
  // Reivindica o bônus de login diário (Nível 2). Server-side decide se
  // já foi reivindicado hoje — o cliente nunca credita saldo por conta própria.
  const claimDailyReward = useCallback(async () => {
    if (!hasSupabase || !user) return { claimed: false, new_balance: null, amount: 0 }
    const { data, error } = await supabase.rpc('claim_daily_reward', { p_user_id: user.id })
    if (error) { console.error('[claimDailyReward]', error.message); return { claimed: false, new_balance: null, amount: 0 } }
    const row = Array.isArray(data) ? data[0] : data
    return row || { claimed: false, new_balance: null, amount: 0 }
  }, [user])

  // Destaques de vitórias reais (marketing honesto) — funciona mesmo
  // sem login, é dado público agregado, não expõe nada sensível.
  const fetchTopWins = useCallback(async (limit = 5) => {
    if (!hasSupabase) return []
    const { data, error } = await supabase.rpc('get_top_wins', { p_limit: limit })
    if (error) { console.error('[fetchTopWins]', error.message); return [] }
    return data || []
  }, [])

  return { syncRound, fetchHistory, fetchTransactions, fetchGameStats, fetchPendingWithdrawals, cancelWithdrawal, claimDailyReward, fetchTopWins }
}
